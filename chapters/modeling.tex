...
When Cundall \& Strack first proposed the discrete element method, they used a linear spring-dashpot structure which saw the normal and tangential forces written as,
\begin{subequations}
\label{eq:dem-forces}
\begin{align}
...
The method used in this work fits into the computational skeleton of Cundall and Strack's method but with non-linear spring-dashpot coefficients defined by simplified Hertz-Mindlin-Deresiewicz model. In this model, the normal-direction stiffness coefficient of \Cref{eq:normal-force} is based on the Hertzian contact law (derived explicitly in \Cref{sec:hertz-theory}). The tangential-direction stiffness coefficient follows from Brilliantov \cite{Brilliantov1996, Zhu2007, Langston1995}. Together, the spring coefficients are,
\begin{subequations}
\begin{align}
...
In all past DEM efforts for ceramic breeders, the validity of Hertz theory for describing the inter-particle contact force as a function of fictitious overlap is assumed (see \Cref{sec:modeling-dem}). In our experimental test stand for crushing individual pebbles, shown in \Cref{fig:nfri-fmax}, our equipment was able to record accurate measurements of the force-travel relationship for each pebble, providing us with an opportunity to study the validity of the Hertzian contact laws. 
...
The question then remains, if the force-travel response is fitting Hertzian predictions, why are the experimental curves so different than Hertzian predictions? The phenomena leading to the variation of crush forces seen in \Cref{fig:nfri-max}, we hypothesize, is also behind variation in travel-force curves of \Cref{fig:nfri-exp-curves}. The process of manufacture of pebbles leads to slightly different internal structures in the ceramic. Those differences yield stronger or weaker pebbles in a probability around a mean value, as seen in \Cref{fig:nfri-1-exp-fmax}. Different internal structure is likely also causing each pebble to behave with different stiffness. Thus if the Young's modulus in \Cref{eq:contact-force} for the batch of pebbles had a probability distribution, rather than a single value, we can account for the variations in responses of \Cref{fig:nfri-exp-curves}. 

I introduce an `apparent' Young's modulus for each pebble which is iteratively found to be best fits between \Cref{eq:contact-forces} with pebbles from experiments. The reported value is normalized against the \lit~Young's modulus from literature, $E_\text{lit}$, 
\begin{equation}
...
\Cref{eq:peb_hertz} is a generic translation between lab materials and packed bed materials. We will use the equation as the basis for our pebble crushing prediction in DEM simulations. Before implementing a custom crush-predicting method into the DEM code, I will review other attempts at modeling and predicting crush behavior in packed beds.
...
Thus the number of fragments goes like the inverse of radius ratio to the third power and the number of crushed fragments to represent a single crushed pebble increases rapidly as the fragments shrink. The relationship between radius ratio and number of fragment particles is given in \Cref{fig:fragment-count}. Note that in the DEM simulation, it is impossible to insert fractions of a particle so the number of fragment pebbles is rounded to the nearest integer in the figure.
