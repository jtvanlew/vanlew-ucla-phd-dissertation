...
In a CFD-DEM study of \cite{Swasdisevi2005}, the Ranz-Marshall correlation 
\begin{equation}
...
In a CFD-DEM study by Rickelt\etal,\cite{Rickelt2013} they used correlations from Gnielinski (whcih is also used by Achenbach\cite{Achenbach199517}) which covers $\Re$ up to \num{10e5} and $\Pr$ from \numrange{0.7}{7e4}
\begin{equation}
...
where
\begin{equation}
    f_\epsilon = 1 + 1.5(1-\epsilon)
\end{equation}




Yang \& Garimella, studying solar thermal energy storage used the correlation from Wakao Kaguei. They have\cite{Yang2010}
\begin{equation}
    \Nu = 6(1-\epsilon)(2 + 1.1\Re_L^{0.6}\Pr_L^{1/3})
\end{equation}
where $\Re_L$ and $\Pr_L$ are the local Reynolds and Prandtl numbers.
...
Li \& Mason compared many correlations for Nusselt number for a single particle. They use Ranz \& Marshall correlation up to $\Re = 200$. Then the middle range is a modified form of Kemp and the last case for large Reynolds number is from a modified Frantz
...
\end{equation}
where the dimensionless grouping $N_v = \frac{\dot{m}D_hc_f}{\epsilon A_tk_f}$. They used a modified Reynolds number based on the mass flux and hydraulic diameter of their storage tank,
\begin{equation}
    Re_G=\frac{4 \dot{m} r_{char}}{\epsilon \pi R_t^2\mu_f}
\end{equation}
where $R_t$ is the radius of the container and with $r_char$ a characteristic radius (or hydraulic radius) of their packing, 
\begin{equation}
...
Bandrowski \& Kaczmarzyk\cite{Bandrowski1977} investigated gas-to-particle heat transfer for pneumatically conveyed spherical particles, their data fit the relationship
\begin{equation}
     \Nu = 0.00114\epsilon^{-0.5984}\Re_p^{0.8159}
\end{equation}
Although Bandrowski \& Kaczmarzyk\cite{Bandrowski1977} is only for extremely dilute flow at high particle Reynolds number ($180 < \Re_p < 1800$ and $0.00025 < \phi < 0.05$).
...
Visser's thesis notes \cite{Visser2007} that the correlation of Gnielinksi is valid for $\Re$ in \numrange{500}{1000}, for $\epsilon$ \numrange{0.26}{1.0}. Achenbach correlation is for $\Re > 1$ and $\epsilon = 0.387$. So he uses Gunn 
\begin{equation}
    \Nu_p = (7-10\epsilon+5\epsilon^2)(1+0.7\Re_p^{0.2}\Pr^{1/3})+(1.33-2.4\epsilon+1.2\epsilon^2)\Re_p^{0.7}\Pr^{1/3}
\end{equation}
Amritkar\etal also used\cite{Amritkar2014} a form given by Gunn in 1978
\begin{equation}
    \Nu = (7-10\epsilon+5\epsilon^2)(1+0.7\Re_p^{0.2}\Pr^{1/3})+(1.33-2.4\epsilon+1.2\epsilon^2)\Re_p^{0.7}\Pr^{1/3}
\end{equation}


Zhou\etal\cite{Zhou2009} use the form
\begin{equation}
    \Nu_i = 2+a\Re_i^b\Pr^{1/3}
\end{equation}
then from Kunii and Levenspiel, choose $b =0.5$ and hvae $a$ in the range of 0.6 to 1.8. They cite literate of bed conditions similar to their own to choose $a = 1.2$, thus
\begin{equation}
    \Nu_i = 2 + + 0.2\Re_i^{1/2}\Pr^{1/3}
\end{equation}
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% new subsection
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
