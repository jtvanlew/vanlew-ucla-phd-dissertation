...
In order to quantify radiative heat transfer compared to conduction heat transfer, consider particle $i$ at temperature $T_i$ with material properties of $E$, $\nu$, radius $R$, emissivity $\epsilon$, and solid conductivity $k_s$ in contact with particle $j$ of the same material at temperature $T_j$, the amount of heat transfered through a single contact of these two pebbles is
\begin{equation}
Q_c = 2k_s\left(\frac{3}{4}\frac{R^*}{E^*}\right)^{1/3}F_n^{1/3} (T_i - T_j)
\end{equation}
Assuming further that the contact force between these two particles is about \SI{10}{\newton}. Radiation exchange from particle $i$ to $j$ can be found from a grey-body radiative transfer equation,
\begin{equation}
Q_r = \epsilon\sigma A_s\left(T_i^{1/4} - T_j^{1/4}\right)
\end{equation}
where we will assume that the surface area projected between the two particles is a half-sphere, $A_s = 2\pi R^2$.

The amount of energy transfered between the two particles is a function of not only the temperature differences but also the absolute temperatures. In the following figures, the temperature of $T_i$ is varied between \SIrange{300}{800}{\celsius} and $T_j$ is varied between $T_i - 100$ to $T_i$. The amount of heat transfer is normalized against the amount of heat transfer between the pebbles due to conduction at the given temperature (\textit{i.e.} $Q_c$ will always equal unity). The results, given in \Cref{fig:qc-vs-qr}, show that when average pebble temperatures are above \SI{600}{\celsius}, the amount of energy transfered between these two pebbles will be approximately half of that due to contact conductance. When pebble $i$ increases to \SI{800}{\celsius}, the heat transfer between pebbles of comparable temperature is almost equally divided between radiation and contact conductance.
...
Temperatures expected in the solid breeder will range between, approximately, \SI{300}{\celsius} and \SI{800}{\celsius}. And as such, there will be regions where radiation is negligible and regions where radiation is just as important to heat transfer as conduction. Radiation exchange between contacting neighbors in a packed bed becomes extremely complex due to the local and semi-local nature of radiation. A standard approach to treat radiation exchange between surfaces is to consider the view factor between them. In a dense, randomly packed bed of spheres the computation of view factors between pebbles can be done \textit{via} a method such as that proposed by Feng and Han \cite{Feng2012}. Unfortunately, in the current implementation of DEM, ray-tracing of view factors is not accomplished and no radiation heat transfer is calculated. Radiation is one of the most important features that should be included in future iterations of DEM modeling.
