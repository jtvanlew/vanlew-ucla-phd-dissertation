...
Then using \Cref{eq:separationh}, we have an expression for the elastic displacements as
...
For solids of revolution, a distribution of pressure to satisfy the displacements of \Cref{eq:hertz-disp-2} is proposed by Hertz as
...
This is applied to both bodies and plugged into \Cref{eq:hertzdisplacements} to yield

\begin{equation}\label{eq:pressindisplacement}
	\frac{\pi p_0}{4aE^*}\left(2a^2 - r^2\right) = \delta - \left(\frac{1}{2R^*}\right)\, r^2
\end{equation}

where we have introduced the now-common term of pair Young modulus,
...
With the solution of \Cref{eq:pressindisplacement}, if we consider $r = a$ and $\delta(a) = 0$, we find the radius of the contact circle is
...
The radius, overlap, and pressure relations are inserted into \Cref{eq:hertzforcewithpressure} to find the force (from now on referred to as the Hertz force) as a function of overlap, relative radius, and pair Young modulus,
...
In which case the force of \Cref{eq:hertz-normal-force} is calculated from material and geometric properties alone and no phenomenological, empirical fits are necessary.
