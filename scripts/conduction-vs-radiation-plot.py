...
for i, T in enumerate(T1):
    ...
    plt.figure(i)
    plt.plot(T2[0:-1], Qc[0:-1], linewidth = 2, color = color_idx[0], label='Conduction')
    plt.plot(T2[0:-1], Qr[0:-1], linewidth = 2, color = color_idx[1], label="Radiation")
    ...
    plt.legend(loc='best')
    plt.grid('on')
    plt.savefig('../figures/conduction-vs-radiation/%s.png'%(max(np.round(T2,0))), format='PNG')
