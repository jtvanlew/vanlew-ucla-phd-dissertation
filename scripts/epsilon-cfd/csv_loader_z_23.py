...
fig, ax = plt.subplots(2,2,figsize=(15,7), dpi=200,)# sharey=True)
fig2, ax2 = plt.subplots(2,2,figsize=(15,7), dpi=200,)# sharey=True)
...
for i, filename in enumerate(prefixes):
	...
	print(filename + " largest local percent increase in packing fraction = %s"%(phi_diff.max()/.62))
	print(filename + " largest local percent decrease in packing fraction = %s"%(phi_diff.min()/.62))
	# Interpolate
...
cNorm = mpl.colors.Normalize(vmin=vmin, vmax=vmax)
#-- Creating a new axes at the right side
ax4 = fig2.add_axes([0.9, 0.1, 0.03, 0.8])
#-- Plotting the colormap in the created axes
cb1 = mpl.colorbar.ColorbarBase(ax4, norm=cNorm)
fig2.subplots_adjust(left=0.1,right=0.85)
