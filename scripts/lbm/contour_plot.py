...
x_grid = np.linspace(-3, 3, 241)
y_grid = np.linspace(-2, 2, 160)

X, Y = np.meshgrid(x_grid, y_grid)

z_slices = np.linspace(0, 367, 368)

v = np.sqrt(df['velocity:0']**2 + df['velocity:1']**2 + df['velocity:2']**2)/0.05
levels = np.linspace(0, 25, 200, endpoint=True)
ticks = np.linspace(0, 25, 15, endpoint = True)
...
for k, z in enumerate(z_slices):
	...
	pngFile = 'images/contour-%s.png'%(k)
	plt.savefig(pngFile)
plt.show()